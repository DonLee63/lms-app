...
import 'package:study_management_app/models/essay_question.dart';
class EditEssayQuizScreen extends ConsumerStatefulWidget {
  final int quizId;
  final int hocphanId;

  const EditEssayQuizScreen({
    super.key,
  ...
  @override
  ConsumerState<EditEssayQuizScreen> createState() => _EditEssayQuizScreenState();
}

class _EditEssayQuizScreenState extends ConsumerState<EditEssayQuizScreen> {
  ...
  int _userId = 1;
  late Future<List<TuluanCauhoi>> _questionsFuture;
  late Future<BodeTuluan> _quizFuture;

  @override
  void initState() {
    super.initState();
    debugPrint('Timezone: ${DateTime.now().timeZoneName}, offset=${DateTime.now().timeZoneOffset}');
    _questionsFuture = Future.value([]);
    _quizFuture = ref.read(exerciseRepositoryProvider).getEssayQuizById(widget.quizId);
    _loadUserId();
  }
    ...
    try {
      ...
      setState(() {
        ...
        _selectedQuestions = quiz.questions
            ...
            .toList();
        debugPrint('Loaded essay quiz: startTime=$_startTime, endTime=$_endTime');
      });
    } catch (e) {
      if (context.mounted) {
  ...
  Future<void> _submitQuiz() async {
    ...
    if (_formKey.currentState!.validate() &&
      ...
      try {
        debugPrint('Submitting essay quiz: local startTime=${quiz.startTime}, endTime=${quiz.endTime}');
        final updatedQuiz = await ref.read(updateEssayQuizProvider(quiz).future);
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: const Text("Cập nhật đề thi tự luận thành công!"), backgroundColor: Colors.green[700]),
          );
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(context);
              try {
                await ref.read(deleteEssayQuestionProvider(questionId).future);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Xóa câu hỏi tự luận thành công'),
